...
export const generateShortUrl = async (req: Request, res: Response): Promise<void> => {
  ...
  await setCache(id, { originalUrl, shortUrl, enabled: true, hits: 0 }, CACHE_TTL);
...
export const redirectUrl = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!data) {
    ...
    const url = await Url.findOne({ shortUrl: `http://localhost:3000/${id}`, enabled: true });
    if (!url) {
    ...
    data = { originalUrl: url.originalUrl, shortUrl: url.shortUrl, enabled: url.enabled, hits: url.hits };
  ...
  Url.updateOne({ shortUrl: data.shortUrl }, { $inc: { hits: 1 }, $set: { lastAccessed: new Date() } }).exec();

  res.redirect(data.originalUrl);
};

// Actualiza el estado (habilitar/deshabilitar) de la URL
export const updateUrlStatus = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { enabled } = req.body;

  // Actualiza en MongoDB
  const url = await Url.findOneAndUpdate({ shortUrl: `http://localhost:3000/${id}` }, { enabled }, { new: true });
  if (!url) {
  ...
  res.json({ message: `URL ${enabled ? 'habilitada' : 'deshabilitada'} correctamente.` });
};

// Modifica la URL de destino
export const updateOriginalUrl = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { originalUrl } = req.body;

  // Actualiza en MongoDB
  const url = await Url.findOneAndUpdate({ shortUrl: `http://localhost:3000/${id}` }, { originalUrl }, { new: true });
  if (!url) {
    res.status(404).json({ error: 'URL no encontrada' });
    return;
  }
